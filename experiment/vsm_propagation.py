...
def vsm(fp=CORPUS_FILE):
    with open(fp, 'r') as json_file_in:
        ...
        annoy_index.save('vsm_euclidean.ann')
...
def closest_topK(unseen_event, ids_dict, model, dim, topK=10):
    ...
    annoy_index = AnnoyIndex(dim, 'euclidean')
    annoy_index.load('vsm_euclidean.ann')
...
def embedding_propgation(ranking_list, fp=EMBEDDING_FILE):
    ...
    for ranking_list_index, (id_, score) in enumerate(ranking_list):
        try:
            ...
            continue
        if add_count == 0:
            accumulate_vector = list(map(lambda x: x * (1 / (0.00001 + score)), added_vector))
        else:
            for index, (element1, element2) in\
                            enumerate(zip(accumulate_vector, added_vector)):
                accumulate_vector[index] = element1 + element2 * (1 / (0.00001 + score))
        add_count += 1
        accumulate_weight += 1 / (0.00001 + score)
...
if __name__ == "__main__":
    ...
    for id_, title_string in UNSEEN_DICT.items():
        ...
        print()
    with open('unssen_events_rep_hpe(tf_weight_euclidean).txt', 'wt') as fout:
